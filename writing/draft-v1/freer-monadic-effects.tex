...
To \ep{interleave} effects is to use multiple effects at the level.
...
A \kw{freer-monadic handler}, or just \kw{handler}, has a type of the form \code|freer $υ$ $α$ → $ω$ $α$|, where $ω$ is the type of affected results (parametrized by the result type $α$).
